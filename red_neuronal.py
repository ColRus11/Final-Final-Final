    ...
    def entrenar_mlp(model, data, labels, epochs, lr):
        ...
        for epoch in range(epochs):
            ...
            for x, y in zip(data, labels):
                ...
                # Backward pass with binary cross-entropy
                output = activations[-1]
                # Delta for output layer: output - y (cross-entropy gradient)
                deltas = [output[i] - y if i == 0 else 0 for i in range(len(output))]

                # Compute loss for logging (optional)
                if abs(output[0]) < 1e-10: output[0] = 1e-10  # Avoid log(0)
                if abs(1 - output[0]) < 1e-10: output[0] = 1 - 1e-10
                loss = -(y * Matematicas.log(output[0]) + (1 - y) * Matematicas.log(1 - output[0]))
                total_loss += loss
            ...
            # Log average loss every 1000 epochs (optional)
            if epoch % 1000 == 0:
                avg_loss = total_loss / len(data)
                print(f"Epoch {epoch}, Avg Cross-Entropy Loss: {avg_loss}")
